"""
facultyBot/main.py
Faculty Assistant Chatbot — FastAPI Backend  (multi-tenant)

Works for ANY business domain (restaurant, clinic, shop, brand, etc.).
Bot name + knowledge base = auto domain detection.

Multi-tenant: one process now serves EVERY "faculty" category bot.
...
def _get_bot_ctx(bot_id: int) -> dict:
    ...
    print(f"[facultyBot] cached id={bot_id} name='{ctx['bot_name']}' org='{ctx['org_name']}'")
    print(f"[facultyBot] index='{p_index}' namespace='{p_ns}' FAQs={len(faqs)}")
...
def _norm_name(name: str) -> str:
    """Normalize a name for dedup purposes: lowercase, strip common honorifics,
    collapse whitespace. 'Dr. Zulfiqar Ahmed' and 'Zulfiqar Ahmed' should be
    treated as the same person even though they're separate Pinecone chunks
    with inconsistent name strings."""
...
def _build_image_url(image_path: str) -> str:
    """Turn a stored path like 'uploads\\35\\images\\x.jpg' into an absolute
    URL the browser can actually load. Images are served by the main
    SmartChat API, not this bot process."""
    if not image_path:
        return ""
    normalized = image_path.replace("\\", "/").lstrip("/")
    return f"{_SMARTCHAT_API.rstrip('/')}/{normalized}"


async def _retrieve_context(vector_store, query: str, k: int = 8):
    """Retrieve relevant chunks from Pinecone, then keep only the ones that
    are genuinely close to the best match — not just the raw top-k.
    similarity_search() always returns exactly k results even when only one
    is actually relevant (e.g. "who is zuhaib" was pulling in 4 unrelated
    faculty who simply happened to be the next-nearest vectors). Gating by
    score relative to the top hit fixes that, while still allowing multiple
    results through for genuine list-style queries ("list all lecturers in
    CS"), since those chunks score similarly close to each other.

    Returns (context_str, people):
      - context_str: formatted text block for the LLM prompt (unchanged behavior)
      - people: deduped list of faculty profile dicts built straight from
        Pinecone metadata (name/department/.../image_path -> image URL).
        Built here, from retrieved metadata, NOT from the LLM's own text —
        the LLM should never be trusted to reproduce image paths or emails
        accurately.
    """
    # How far below the top score a result can be and still count as
    # relevant. Smaller = stricter (fewer, more on-topic results).
    # Tune against your actual embedding model / Pinecone metric —
    # this assumes a cosine-similarity index (higher score = more similar).
    RELEVANCE_MARGIN = 0.08

    loop = asyncio.get_event_loop()
    try:
        docs_and_scores = await loop.run_in_executor(
            None, lambda: vector_store.similarity_search_with_score(query, k=k)
        )
        if not docs_and_scores:
            return "", []

        docs_and_scores.sort(key=lambda ds: ds[1], reverse=True)
        top_score = docs_and_scores[0][1]
        relevant = [d for d, s in docs_and_scores if s >= top_score - RELEVANCE_MARGIN]

        context = "\n\n".join(
            f"[Chunk {i+1}] {d.page_content}"
            for i, d in enumerate(relevant) if d.page_content.strip()
        )
        ...
        for d in relevant:
            ...
            people.append({
                ...
                "image":         _build_image_url(meta.get("image_path", "")),
            })
...
async def _run_chat(ctx: dict, prompt: str, history: list[dict]) -> dict:
    ...
    # Step 1: retrieve
    context, people = await _retrieve_context(ctx["vector_store"], prompt)
    ctx_block = (
        f"\n\n--- Document Excerpts ---\n{context}\n---"
        if context else ""
    )
    ...
    # Only show a card for someone the reply text actually names. Retrieval
    # (even after relevance-gating) will legitimately pull in people who are
    # semantically close but not who was asked about — e.g. asking for HODs
    # can retrieve their deputies/coordinators too, since those chunks live
    # right next to the HOD's in the embedding space. Cross-checking against
    # what the LLM actually wrote keeps the cards in sync with the text.
    final_text   = (display or raw).lower()
    mentioned    = [p for p in people if _norm_name(p["name"]) in final_text]
    show_people  = bool(mentioned) and not no_context
    ...
    return {
        ...
        "type":          "faculty" if show_people else None,
        "people":        mentioned if show_people else [],
    }
...
app = FastAPI(title="SmartChat Faculty Bot API", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
